...
const getStudentProfile = async (req, res) => {
  try {
    ...
    if (!student) return res.status(404).send({ status: 'Error', msg: 'Student not found' })
    res.send(student)
  } catch (error) {
...
const updateStudentProfile = async (req, res) => {
  try {
    const student = await Student.findByIdAndUpdate(res.locals.payload.id, req.body, { new: true }).select('-passwordDigest') 
    if (!student) return res.status(404).send({ status: 'Error', msg: 'Student not found' })
    res.send(student)
  } catch (error) {
